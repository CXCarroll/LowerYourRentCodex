	...
	{#if committed}
		<div class="card border-brand-200 bg-brand-50 p-6">
			...
			<div class="mt-4 flex gap-3 text-sm">
				...
				<a href="/admin/upload/fmr" class="btn-primary">Upload another</a>
			</div>
		</div>
	{:else}
		<form
					...
					<table class="w-full text-sm">
						<thead class="bg-slate-50 text-xs text-slate-500 uppercase">
							<tr>
								...
								<th class="px-6 py-2 text-left font-medium">County FIPS</th>
								<th class="px-6 py-2 text-left font-medium">Apt type</th>
								<th class="px-6 py-2 text-right font-medium">FMR</th>
							</tr>
						</thead>
						<tbody>
							{#each preview.sample as r (r.year + '|' + r.countyFips + '|' + r.aptType)}
								<tr class="border-t border-slate-100">
									<td class="px-6 py-2 tabular-nums text-slate-900">{r.year}</td>
									<td class="px-6 py-2 font-mono text-slate-700">{r.countyFips}</td>
									<td class="px-6 py-2 text-slate-700">{r.aptType}</td>
									<td class="px-6 py-2 text-right tabular-nums text-slate-900">
										{fmtDollars(r.fmrCents)}
									</td>
								</tr>
							{/each}
						</tbody>
